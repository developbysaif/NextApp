...
export default function AdminBlogDetailPage({ params }) {
    ...
    useEffect(() => {
        // Try matching by numeric ID first (from /admin/blogs/[id])
    ...
    return (
        <div className="font-sans text-gray-800 bg-[#FDFBF7] min-h-screen w-full pb-24">
            ...
            <div className="w-full bg-white/95 backdrop-blur-sm px-6 md:px-10 py-4 sticky top-0 z-50 shadow-sm border-b border-gray-100 flex items-center justify-between">
                <button onClick={() => router.push('/admin/blogs')} className="flex items-center gap-2 text-gray-500 hover:text-gray-900 font-bold text-xs transition-colors">
                    <ChevronLeft size={18} strokeWidth={2.5}/>
                    <span className="hidden sm:inline">Back to Menu</span>
                </button>
                <span className="text-[10px] font-bold tracking-widest uppercase text-gray-400">Nutrigo · Admin Dashboard</span>
